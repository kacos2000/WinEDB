...
try
{
	...
	Write-Host "[Microsoft.Isam.Esent.Interop.Api] exists." -f Gray
}
catch
{
...
'@
	...
	Write-Host "[Microsoft.Isam.Esent.Interop.Api] version 2.0.2 loaded OK." -f DarkCyan
}

$OpenFileDialog = New-Object System.Windows.Forms.OpenFileDialog
$OpenFileDialog.AutoUpgradeEnabled = $true
$OpenFileDialog.InitialDirectory = "$([Environment]::GetFolderPath('Desktop'))"
$openfiledialog.Filter = "Windows.edb (Windows.edb)|Windows.edb*|All files (*.*)|*.*"
...
{
	$SourcedbFile = $OpenFileDialog.FileName
...
{
	Write-Host "You did not select a valid Windows.edb file." -f Red
	Return
}
...
if (Test-Path $outfile -IsValid)
{
	...
	$dbfile = $outfile
	Write-Host "Finished copying the dB to $($env:TEMP)"
}
else
{
	Write-Host "Error copying the dB to $($env:TEMP)" -f Red
	return
}
...
Write-Host "Database size: $($FileSize.ToString('N0'))" -f Yellow
if ($FileSize -le 0)
{
	Write-Host "Could not read edb size. Something went wrong." -f Red
	return
}
...
catch
{
	...
	$stopWatch.Reset()
	Stop-Transcript
	Write-Host "Could not Attach the temp dB $($dbfile)" -f Red
...
catch
{
	...
	$stopWatch.Reset()
	Stop-Transcript
	Write-Host "Error openinfg ESE dB" -f Red
	# Delete temp copy of the dB
	[System.IO.File]::Delete($dbfile)
	return
}

# Will be used to read Filetimes
$dates = @(
	'DateModified'
	'DateCreated'
	'DateAccessed'
...
$dBRecords = [System.Collections.Specialized.OrderedDictionary]::New()

$TableData = foreach ($TableName in $TableNames)
{
	# Set Table
	...
	try
	{
		...
		if ($RecCount -lt 1)
		{
			write-host "Skipping $($TableName) - Records: $($RecCount)" -f Red
			continue
		} # skip table
	}
	catch
	{
		Write-Host "Skipping $($TableName) - Table is empty" -f red
		continue # skip table
	}
	...
	write-host "Table $($TableName) - " -f cyan -nonewline
	Write-Host "Columnns: $($ColumnCount) - Records: $($RecCount)" -f White
	if ($RecCount -lt 1) { continue } # Skip empty Table
...
Foreach ($dbtable in $TableData)
{
	...
	Write-Host "Exported table '$($dbtable.Table)" -f DarkGreen
}
Write-Host "Exported CSV are at: '$($outfolder)'" -f Yellow


#close/detach the database
try
{
	Write-Host "Shutting down database $edbfile due to normal close operation." -f White
	...
	# Delete temp copy of the dB
	[System.IO.File]::Delete($dbfile)
	Write-Host "Completed shut down successfully." -f White
	$stopWatch.Stop()
	$stopWatch.Reset()
	Stop-Transcript
}
...
{
	$stopWatch.Stop()
	Write-Host "Processing of $($File) finished in: $($stopWatch.Elapsed.Hours.ToString('0#')):$($stopWatch.Elapsed.Minutes.ToString('0#')):$($stopWatch.Elapsed.Seconds.ToString('0#'))." -f Yellow
	Write-Host "The dB Tables were exported as CSVs to $($outfolder)"
	$stopWatch.Reset()
	Stop-Transcript
	# Delete temp copy of the dB
	[System.IO.File]::Delete($dbfile)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC