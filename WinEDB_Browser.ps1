...
function Show-MainForm_psf
{
	...
	function Get-TextFromBytes
	{
		...
		if ($rawbytes.length -eq 0) { return $null }
		elseif($rawbytes.length -ge 4)
		{
			# text strings with bom
			switch ($rawbytes[0])
			{
				0xFF {
					if ($rawbytes[1] -eq 0xFE) { [System.Text.Encoding]::Unicode.GetString($rawbytes); break } # unicode
				}
				0xEF {
					if ($rawbytes[1] -eq 0xBB -and $rawbytes[2] -eq 0xBF) { [System.Text.Encoding]::UTF8.GetString($rawbytes); break } # utf8
				}
				0xFE {
					if ($rawbytes[1] -eq 0xFF) { [System.Text.Encoding]::BigEndianUnicode.GetString($rawbytes); break } # BE unicode
				}
				0x00 {
					if ($rawbytes[1] -eq 0x00 -and $rawbytes[2] -eq 0xFE -and $rawbytes[3] -eq 0xFF) { [System.Text.Encoding]::UTF32.GetString($rawbytes); break } # utf32
				}
				0x2B {
					if ($rawbytes[0] -eq 0x2B -and $rawbytes[1] -eq 0x2F -and $rawbytes[2] -eq 0x76) { [System.Text.Encoding]::UTF7.GetString($rawbytes); break } #utf7
				}
				{ $_ -in (0x20 .. 0x7F) }{
					if ($rawbytes[1] -eq 0x00 -and $rawbytes[2] -in (0x20 .. 0x7F) -and $rawbytes[3] -eq 0x00) 	{ [System.Text.Encoding]::Unicode.GetString($rawbytes); break } # unicode
				elseif ($rawbytes[1] -in (0x20 .. 0x7F) -and $rawbytes[2] -in (0x20 .. 0x7F) -and $rawbytes[3] -in (0x20 .. 0x7F)) { [System.Text.Encoding]::UTF8.GetString($rawbytes); break } # ASCII/UTF8
				else { [System.BitConverter]::ToString($rawbytes).Replace('-', '') }
				}
				default
				{
					[System.BitConverter]::ToString($rawbytes).Replace('-', '')
				}
			}
		}
		else
		{
			if ($rawbytes[0] -in (0x20 .. 0x7F) -and $rawbytes[1] -eq 0x00) # unicode
			{ [System.Text.Encoding]::Unicode.GetString($rawbytes) }
			elseif ($rawbytes[0] -in (0x20 .. 0x7F) -and $rawbytes[1] -in (0x20 .. 0x7F)) # ASCII
			{ [System.Text.Encoding]::UTF8.GetString($rawbytes) } # Should play well with ASCII too
			else { [System.BitConverter]::ToString($rawbytes).Replace('-', '') } # Just get the Hex String	
		}
		
	} # End of Get-TextFromBytes
	
	function GetText-fromHex
	{
		param
		(
			[Parameter(Mandatory = $true)]
		...
		if ([System.String]::IsNullOrEmpty($Hex)) { return $null }
		try
		{
			# Check if string is valid Hex
			[System.Text.RegularExpressions.Regex]$regex = '^[a-fA-F0-9]+$' #'\p{XDigit}'
			if ($regex.Matches($Hex).Success -and $regex.Matches($Hex).length -eq $Hex.Length)
			{
				# Convert String to Byte Array
				$bytes = ($Hex -Split '(..)' -ne '').foreach{ [int]"0x$($_)" }
				# Try to Convert the Byte Array to Text
				$text = Get-TextFromBytes -RawBytes $bytes
				# Check if the returned Text is Valid ASCII			
				[regex]$regex2 = '^[\x20-\x7F]+$'
				if ($regex2.Matches($text).Success -and $regex2.Matches($text).length -eq $text.Length)
				{ $text }
				else { $null }
			}
			else { $null }
	...
	$Open_Click={
		...
		$openfiledialog1.Title = "Please select a Windows.EDB file"
		if ($openfiledialog1.ShowDialog() -eq 'OK')
		{
			...
			$WinEDBBrowser.Text = "WinEDB Browser - $($fname)"
		...
		$data = if ($null -ne $Binarydata)
		{
			switch ($column.Coltyp)
			{
				...
				'LongBinary'{
					...
					if ($column.name -match $ulist -and $column.name -match '^[\d]{2}')
					{
	...
	function Read-EDB
	{
		...
		try { [System.IO.File]::Copy($SourcedbFile, $outfile, $true) }
		catch{ invoke-expression ("esentutl.exe /y /vss " + "$($SourcedbFile)" + " /d " + $($outfile) + " /o") }
		...
		# Create Instance
		[Microsoft.Isam.Esent.Interop.Api]::JetCreateInstance2([ref]$Instance, 'WinEDB[0]', 'WinEDB[0]', [Microsoft.Isam.Esent.Interop.CreateInstanceGrbit]::None)
		...
		$SaveRowData = {
			...
			# Just in case one deletes the Output folder while WinEdb is running
			$outpath = Split-Path $rPath -Parent
			$filename = Split-Path $rPath -Leaf
			if (!(Test-Path -Path $outpath))
			{
				try { $null = [System.IO.Directory]::CreateDirectory("$($outpath)") }
				catch { $rPath = "$($env:TEMP)\$filename" }
			}
		...
		try
		{
			...
			$Status.Text = "$($Error[0].Exception.InnerException | Out-String) - [1] Will attempt to repair with Esentutl"
			[Microsoft.Isam.Esent.Interop.Api]::JetTerm2($Instance, [Microsoft.Isam.Esent.Interop.TermGrbit]::Complete)
			try
			{
				# Commands
				$ese_repair = "esentutl.exe /p " + "$($dbfile)" + " /g /o"
				$ese_defrag = "esentutl.exe /d " + "$($dbfile)" + " /o"
				# repair
				$Status.Text = "[1] Attempting repair of $($dbfile) with Esentutl /p"
				invoke-expression -Command $ese_repair -OutVariable Esentrepairlog
				# defragment
				$Status.Text = "[1] Attempting defragmentation of $($dbfile) with Esentutl /d"
				invoke-expression -Command $ese_defrag -OutVariable Esentdefraglog
				# New Session
				$Session = [Microsoft.Isam.Esent.Interop.JET_SESID]::Nil
				$syspath = "$($env:TEMP)\$([IO.Path]::GetRandomFileName())"
				$logpath = "$($env:TEMP)\$([IO.Path]::GetRandomFileName())"
			...
			catch
			{
					...
					$stopWatch.Reset()
					$Status.Text = $null
					Show-ErrorMessage -ErrorMessage "[2] Could not Attach the temp copy of $($SourcedbFile):`n $(($Error[0].Exception.InnerException | Out-String))`nTry repairing it with 'esentutl.exe'"
		...
		try
		{
			...
			$TableNames.Insert(0, 'MSysObjects') # Manually Insert the System Tables (hidden from the Interop API)
		...
		# Export Esent repair/defrag info (if it exists) to show that it was executed	
		if (!!$Esentrepairlog) { $Esentrepairlog | Out-File -FilePath "$($outfolder)\EsentUtl_log.txt" -Encoding utf8 }
		if (!!$Esentdefraglog) { $Esentdefraglog | Out-File -FilePath "$($outfolder)\EsentUtl_log.txt" -Encoding utf8 -Append }
		...
		# Get Paths from the Gather Path table First (if the table exists) to build the Tree
		if ($TableNames -contains 'SystemIndex_GthrPth') 
		{
			...
			While ([Microsoft.Isam.Esent.Interop.Api]::TryMoveNext($Session, $Table))
			{
				...
				foreach ($column in $ColumnArray)
				{
					...
					$data = switch ($column.Coltyp)
						...
						default { [System.BitConverter]::ToString($Binarydata).Replace('-', '') }
					}
			...
			$Status.Text = "Exporting table info.."
			...
			$Parentdict['1'] = [System.Collections.ArrayList]::new() # Add Root
			$script:r = 0
		...
		foreach ($TableName in $TableNames)
		{
			...
			try
			{
				if ($TableName -in $TablesToKeep)
				{
					...
					{
						$idx | Out-File -FilePath "$($outfolder)\$($TableName)_IndexInfo.txt" -Encoding utf8
						...
						$indxes | Out-File -FilePath "$($outfolder)\$($TableName)_IndexInfo.txt" -Encoding utf8 -Append
					}
				}
			}
			catch [System.Management.Automation.MethodInvocationException] { }
			...
			elseif ($RecCount -lt 1 )
			{
				$Status.Text = "Skipping '$($TableName)'  - Records: $($RecCount)"
				continue
			} 
			elseif ($TableName -eq 'SystemIndex_GthrPth') # skip table
			{
				$Status.Text = "Skipping '$($TableName)'  (Has been parsed already)"
				continue
			}
			elseif ($TableName -eq 'SystemIndex_PropertyStore')
			{
				...
				While ([Microsoft.Isam.Esent.Interop.Api]::TryMoveNext($Session, $Table))
				{
					...
					if (!!$SystemKind){
						$colk = [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumn($Session, $Table, $SystemKind, [Microsoft.Isam.Esent.Interop.RetrieveColumnGrbit]::RetrieveIgnoreDefault, $null)
				...
				{
					$Status.Text = "Reading Table: '$($TableName)' - Collecting 'System_Search_Store' Entries of type: '$($stype)'"
					if ($stype -eq 'mapi16')
					{
					...
					foreach ($sit in $SystemItemTypes) 
					{
						...
						$progressbar1.Maximum = $ItemTypesHashTable[$sit].count - 1
						$progressbar1.Value = 0
						$progressbar1.Visible = $true
						$RowRecords = [System.Collections.ArrayList]@()
						foreach ($workIDvalue in $ItemTypesHashTable[$sit])
						{
							...
							if (($r % 50) -eq 0)
							{
								$p = ($r/$ItemTypesHashTable[$sit].count).tostring('P0')
								$progressbar1.Text = $p
								$progressbar1.PerformStep()
								$Status.Text = "Please wait - Reading Table: '$($TableName), with $($ItemTypesHashTable[$sit].count) [$($stype)] $($sit) Records - $($p)"
							}
							...
							$dBData[[System.String]$workIDvalue] = $columndata
							$null = $RowRecords.Add($columndata)
							$r = $r + 1
						} #end rows
						...
						$rpath = Get-SafeFilename -Path "$($outfolder)\$($stype)_$($sit)_Records.csv"
						Start-Job -Name $TableName -ScriptBlock $SaveRowData -ArgumentList $rpath, $RowRecords
					} # end foreach SIT
			...
			$Status.Text = "Exporting table info.."
			# $dBRecords["$($TableName)"] $RowRecords| Export-Csv -Path "$($outfolder)\$($TableName).csv" -Delimiter '|' -Encoding UTF8 -NoTypeInformation
			$TableData.where{ $_.Table -eq $TableName } | select -Property Table, Records | Format-List | Out-File -FilePath "$($outfolder)\$($TableName).txt" -Encoding utf8
			$TableData.where{ $_.Table -eq $TableName } | select -ExpandProperty Columns | Format-Table -AutoSize | Out-File -FilePath "$($outfolder)\$($TableName).txt" -Encoding utf8 -Append
			[System.GC]::Collect()
		} # End foreach Table
		$progressbar1.Value = 0
		$progressbar1.Visible = $false
		...
		$InfoMessage = "Processing of $($File) finished in: $($stopWatch.Elapsed.Days.ToString('0#')) $($stopWatch.Elapsed.Hours.ToString('0#')):$($stopWatch.Elapsed.Minutes.ToString('0#')):$($stopWatch.Elapsed.Seconds.ToString('0#')).`nThe dB Tables were exported as CSVs to $($outfolder)."
		Show-InfoMessage -InfoMessage $InfoMessage
		$Status.Text = "Ready - Processing of $($File) finished in: $($stopWatch.Elapsed.Days.ToString('0#')) $($stopWatch.Elapsed.Hours.ToString('0#')):$($stopWatch.Elapsed.Minutes.ToString('0#')):$($stopWatch.Elapsed.Seconds.ToString('0#'))"
		$stopWatch.Reset()
		$RowRecords.Clear()
		[System.GC]::Collect()
	} # End of Read-EDB
...
'@
	...
	#Event Argument: $_ = [System.Windows.Forms.TreeViewEventArgs]
		...
		$datagridview2.DataSource = $null
		$Tag = $_.Node.Tag
		if (!!$Tag -and $null -ne $Tag[0] -and !!$script:dBRecords)
		{
		...
		elseif (!!$Tag -and $null -ne $Tag[1])
		{
			...
			$record = $Tag[1]
			$datagridview1.DataSource = [System.Collections.ArrayList]@($record|sort -Property 'Name')
...
'))
	#endregion
...

...
'))
	#endregion
...

...
AQsDCgENAwsBDgMKAQ0DCwEOAWUBMAEhAfsBpwEvAQAB/wGnAS8BAAH/A0sBjAgAAwkBDAMbASYD
...
'))
	#endregion
...
AAAASUVORK5CYIIL'))
	...
	$progressbar1.RightToLeft = 'Yes'
	$progressbar1.Size = New-Object System.Drawing.Size(950, 22)
	$progressbar1.Visible = $False
	#
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCNcU0VugIgk8yW
# wUDkhLMfl3rCudle//2IW29idTIjcKCCKI0wggQyMIIDGqADAgECAgEBMA0GCSqG
...
# AQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEILiaLI0rDjYF5Fx+YuBNEyGxebiOC+EG
# c56lDRCBrm5+MA0GCSqGSIb3DQEBAQUABIICACyFClV9Obl2XW56URG1s0/eBXTG
# fMBiG02QEdCe5P3L1w+o9WmuYbKWOD/ywTtX3+spNOIfRZGYD4oODQdz+jwKY+LQ
# 1uN7WNT+Bd5yf1OJYLKw6Xh3qd+UAyk4oA66fWYg8GiMO5aWpeJeZJFHsjIBB3ab
# FclmOa+mtEwCGEjbt6VShXr7TI4r+mwrXVwKzRUcylmjEM4lh42zFihqUUrRVt0+
# xyagjIWZ77MiPvKdeZqo3aY1f3w4mCpABhRm9erMyYjIRLBhyhpofHKS8fS61rmi
# bmAXmNUMDJhTs/iXyZE3lQ26Klllmue4jveV/SjkKuES7je/wC5pRw7OVFEcYVE6
# 94Gfnq1aAocgrb0HbIZwUg7xMnZDfWWVuMLf+3c/GE0t5AVD+i3KdzaGAJUYmw6v
# Pnz7wocsszb25AaJ4+Ku5F/iX2L1s7p0guHMHf4U/HZDAg+8AUOVozjKxh9GGDz/
# l2mXHFhBrqSB7olkFaXh8eoHNdTgoKumNNqgKlC7vGMSAiQlpZ/Yj+MbZurKtQWc
# u9yiLY9OyJOZZby2Mnkrg08pmSp4SIXX7bDP4+ZSOO+b2V+/GOy/i9gn6qH2pkrn
# VFJRSx6S6A+e9AGQlu9QKneN0dGKJuK6F5rUi+D+EOYMunqKv9SMGLVOWSOWNF3h
# 2b0FTl6Uwgi5P1WMoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEBMG8wWzEL
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAxMjAxNzA2MjNaMCsGCSqGSIb3DQEJ
# NDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3DQEJBDEi
# BCDPtqeChbnV8S/MGLJC3sDqf5W56gzTt7NSd4aONEVMazCBpAYLKoZIhvcNAQkQ
...
# p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgGErnikPlsZhhl0w7H+MS58KxcfJ
# KUYc9Wi7gbzCC9tLhgL8NXZBYyuAjC2RCQNod97XmviMu0zDY0cdZU3TAaHcnGbz
# gx4amu77DXXNkzA5f8R0o7UbrJ7VdFbXo1yDZXPrh9mc4IJnodKhbckdO+A6y9H3
# EWU0bDqStko+qgLDPF7YxHKDQmX7xGfkFNcP7zhyT2NNsSTqugP/KAgAHQyTKiof
# cGvTS4uSe7sq18THPiiu+B4bhL1Cr58eawbrS4YpxYCOKY9WK7Xf94FeV6C8g//x
# 6YNDpYXeXXCbjmBD09IxQlV+qFSf8qL/On9XBOmVhXfqVZL5u7vcq1NxBCf1o9jx
# BSHGPvSb/+V3FipjisVCT6Dav6xh9hYgcvfEud5F5sApsuQ5eonouTnRcCaI7wIV
# U1jHtgoLqDzSy6NZsWnJZAj9o1PBQhE4W6L6B+cSLlG8Nu4XPKj89lizEJ29612b
# svOYym9JkTA82WNUfVjtiObg97A3I3phhe+aVQ==
