...
'@
	...
	function Read-EDB
	{
		...
		$null = [Microsoft.Isam.Esent.Interop.Api]::JetSetSystemParameter($Instance, [Microsoft.Isam.Esent.Interop.JET_SESID]::Nil, [Microsoft.Isam.Esent.Interop.JET_param]::CircularLog, [int]$false, $null)
		...
		try
		{
			...
			# Attach dB
			[Microsoft.Isam.Esent.Interop.Api]::JetAttachDatabase($Session, $dbfile, [Microsoft.Isam.Esent.Interop.AttachDatabaseGrbit]::ReadOnly)
		}
		catch
		{
			$stopWatch.Stop()
			$stopWatch.Reset()
			Show-ErrorMessage -ErrorMessage "Could not Attach the temp dB $($dbfile)"
		...
		catch
		{
			$stopWatch.Stop()
			$stopWatch.Reset()
			Show-ErrorMessage -ErrorMessage "Could not Open the temp dB $($dbfile)"
		...
		[Microsoft.Isam.Esent.Interop.Table]$Table = New-Object -TypeName Microsoft.Isam.Esent.Interop.Table($Session, $DatabaseId, $TableName, [Microsoft.Isam.Esent.Interop.OpenTableGrbit]::ReadOnly)
		...
		$Status.Text = "Please wait - Reading $($dbfile) Table $($TableName) with $($columnCount) Columns"
		...
		if ([Microsoft.Isam.Esent.Interop.Api]::TryMoveFirst($Session, $Table.JetTableid))
		{
			while ($true)
			{
				...
				foreach ($column in $ColumnArray)
				{
					...
					$data = if ($column.Coltyp.ToString() -eq '14'){ [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsUInt32($Session, $Table.JetTableid, $column.Columnid)}
						...
						else { [System.BitConverter]::ToString([Microsoft.Isam.Esent.Interop.Api]::RetrieveColumn($Session, $Table.JetTableid, $column.Columnid)).Replace('-', '')}
		...
		{
			$Status.Text = "Error Processing $($TableName) - could be empty."
			# Add File Nodes to Tree
			$treeview1.BeginUpdate()
			$Root = $treeview1.Nodes.Add('1', 'Entries')
			$OrphanNode = $treeview1.Nodes.Add('0', 'Orphans')
		...
		foreach ($TableName in $TableNames)
		{
			...
			while ($true)
			{
				...
				$data = if ($column.Coltyp.ToString() -eq 'Binary' -and $column.name.ToString() -match ($dates -join '|'))
							{
								$cellcontents = [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumn($Session, $Table.JetTableid, $column.Columnid)
								if (!!$cellcontents)
								{
								  try { [datetime]::FromFileTimeUtc([Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsUInt64($Session, $Table.JetTableid, $column.Columnid)).ToString("dd/MM/yyyy HH:mm:ss.fffffff") }
								catch { [System.BitConverter]::ToString([Microsoft.Isam.Esent.Interop.Api]::RetrieveColumn($Session, $Table.JetTableid, $column.Columnid)).Replace('-', '') }
								}
							}
							elseif ($column.name.ToString() -eq 'LastModified')
							{
								  try { [datetime]::FromFileTimeUtc("0x$([bitconverter]::ToString([Microsoft.Isam.Esent.Interop.Api]::RetrieveColumn($Session, $Table.JetTableid, $column.Columnid)).Replace('-', ''))").ToString("dd/MM/yyyy HH:mm:ss.fffffff") } 
								catch { $null }
							}
					elseif ($column.name.ToString() -in ('Parent', 'ScopeID')) { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsUInt32($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'UnsignedByte') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsByte($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'Bit') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsBoolean($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq '17') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsUInt16($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq '16') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsGuid($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq '14') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsUInt32($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'Short') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsInt16($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'Long') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsInt32($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'DateTime') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsDateTime($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'Currency') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsInt64($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'IEEESingle') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsFloat($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'IEEEDouble') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsDouble($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'Guid') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsGuid($Session, $Table.JetTableid, $column.Columnid) }
					elseif ($column.Coltyp.ToString() -eq 'Text') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsString($Session, $Table.JetTableid, $column.Columnid, [System.Text.Encoding]::GetEncoding($column.Cp)) }
					elseif ($column.Coltyp.ToString() -eq 'LongText') { [Microsoft.Isam.Esent.Interop.Api]::RetrieveColumnAsString($Session, $Table.JetTableid, $column.Columnid, [System.Text.Encoding]::GetEncoding($column.Cp)) }
					elseif ($column.Coltyp.ToString() -in ('Binary', 'LongBinary'))
					{
						try
						{
							[System.BitConverter]::ToString([Microsoft.Isam.Esent.Interop.Api]::RetrieveColumn($Session, $Table.JetTableid, $column.Columnid)).Replace('-', '')
						}
						catch { }
					}
					$columndata | Add-Member -MemberType NoteProperty -Name "$($column.Name)" -Value $data
		...
		$Status.Text = "Done"
		$treeview1.EndUpdate()
		if (!!$treeview1.Nodes[0].Nodes['Entries'])
		{
			$treeview1.Nodes[0].Nodes['Entries'].Expand()
		}
		...
		Foreach ($dbtable in $TableData)
		{
			...
			$dbtable|select -Property Table, Columns, Records | Format-List | Out-File -FilePath "$($outfolder)\$($dbtable.Table).txt" -Encoding utf8 
		...
		catch { $Status.Text = "Something wrong happened while shutting down :-/" }
		# Delete temp copy of the dB
		try
		{
			[System.IO.File]::Delete($dbfile)
	...
	$Collapse1_Click={
		$treeview1.BeginUpdate()
		if (!!$treeview1.SelectedNode -and $treeview2.SelectedNode.IsExpanded)
		{
...
dAAACAgCAAAAMAAAADAAAAAPAwAAAL6GAAACAAABAAUAQEAAAAEAIAAoQgAAVgAAADAwAAABACAA
...
'))
	#endregion
	...
	$treeview1.LineColor = [System.Drawing.Color]::Gainsboro 
	$treeview1.Location = New-Object System.Drawing.Point(0, 0)
	$treeview1.Margin = '5, 5, 5, 5'
	$treeview1.Name = 'treeview1'
	$treeview1.ShowNodeToolTips = $True
	$treeview1.Size = New-Object System.Drawing.Size(706, 981)
	...
	$splitcontainer3.SplitterDistance = 981
	...
	$datagridview2.Size = New-Object System.Drawing.Size(706, 336)
...

...
'))
	#endregion
	...
	$imagelist1.Images.SetKeyName(1,'folder_open-check.ico')
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC